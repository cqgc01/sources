...
for ($i = 1; $i -le $maxRetries; $i++) {
    ...
    if ($port443) {
        ...
        # Uso de ${i} para evitar error de sintaxis con los dos puntos
...
if (-not $success) {
    ...
    foreach ($p in 80, 443) {
        ...
        if ($c) { 
            Write-Host "Puerto ${p}: OCUPADO por PID $($c.OwningProcess)" -ForegroundColor Red 
        } else { 
    ...
    $events = Get-WinEvent -FilterHashtable @{LogName='Application'; Level=2} -MaxEvents 5 -ErrorAction SilentlyContinue | 
              Where-Object { $_.Message -match "Apache" }
